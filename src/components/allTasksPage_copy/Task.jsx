...
export default function Task({ darkMode, flag, setFlag, selectedTask, setSelectedTask, task, tasks, inx, setCloseFlag, setEditFlag, setDeleteFlag, deleted, setDeleted, doneFlag, setDoneFlag, setPopupPosition, setCurrentTask }) {
  ...
  useEffect(()=>{
    ...
    renderTasksVisibility();
    // => move the switch after close task
    ...
    if(task.task_status === "open"){
      renderUrgencyVisibility(urgency);
      // => after change urgency, change the visibility of the urgencyDiv of the task according to value selected
    }
  ...
  const renderTasksVisibility=()=>{
      if(!switchDivRef || !switchCircleRef || !switchSwitchRef || !taskCompRef || !statusRef || !urgencyRef || !editRef || !deleteRef){
        return;
      }
      if(!darkMode){
        if((inx === selectedTask && switchFlag) || (task.task_status === "closed")){
          ...
          urgencyDivRef.current.style.display = "none";
          ...
          urgencyDivRef.current.style.display = "flex";
          return;
        }
      }
      if(darkMode){
  ...
  return (
    <div ref={taskCompRef} className='task_component'>
        <div className='task_container'>
          <div className="task_controllersDiv">
          ...
          <TaskDetails detailsRef={detailsRef} urgencyRef={urgencyRef} setUrgencyFlag={setUrgencyFlag} urgencyFlag={urgencyFlag} setSelectedTask={setSelectedTask} setUrgency={setUrgency} inx={inx} task={task} statusRef={statusRef} urgencyDivRef={urgencyDivRef}/>
        </div>
